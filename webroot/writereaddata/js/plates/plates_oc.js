 $(document).ready(function(){
 	...
 	$('.file_upload').on('change', function(){
		...
		if(fileFormat == 'pdf' && checkFormat != fileFormat || get_file_ext[1] == 'docx'){			
			$("#error_size_".concat(field_id)).show().text("Please select PDF file only");
			$("#".concat(field_id)).addClass("is-invalid");
			$("#".concat(field_id)).click(function(){$("#error_size_".concat(field_id)).show().text(''); $("#".concat(field_id)).removeClass("is-invalid");});
			$('#'.concat(field_id)).val('');
			value_return = 'false';
		}
		 if(fileFormat == 'kmz' && checkFormat != fileFormat || get_file_ext[1] == 'docx'){
			$("#error_size_".concat(field_id)).show().text("Please select KMZ file only");
			$("#".concat(field_id)).addClass("is-invalid");
			$("#".concat(field_id)).click(function(){$("#error_size_".concat(field_id)).show().text(''); $("#".concat(field_id)).removeClass("is-invalid");});
			$('#'.concat(field_id)).val('');
			value_return = 'false';
	...
	$('#btnSubmit').on('click', function () {		
		...
		for(var i = 1; i <= rowLength; i++){
			...
			if(uploadfile != ''){
				...
				var previewfile = $("#plateTable tbody tr:nth-child("+i+") td:nth-child(4)").find('a').attr('href'); // DB saved uploaded filename
			...
			console.log('uploadFileExe: '+uploadFileExe);
			console.log('uploadedFileFormat: '+uploadedFileFormat);

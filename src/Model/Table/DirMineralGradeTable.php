<?php 
	...
	class DirMineralGradeTable extends Table{
		...
		public function getGradsArrByMin($mineral_name, $return_date = null, $primary_min = null) {
			...
			//print_r($return_date." ".$cutoffDate); exit;
			$gradeArr = $this->find('all')
		...
		//get minral grade bu minaral name
		public function getGradsbyNameForF2($mineralName, $returnDate = null) {
			...
			$result[""] = "Select";
			foreach ($gradeArr as $tmpArr) {
			  $result[$tmpArr['grade_code']] = $tmpArr['grade_name'];
			}
		...
		public function postDataValidationMasters($forms_data)
		{
			...
			$gradeCode = $forms_data['grade_code'];
			$gradeName = $forms_data['grade_name'];

			if ($forms_data['mineral_name'] == '') {
				$returnValue = null;
			}
			if ($forms_data['grade_code'] == '') {
				$returnValue = null;
			}
			if (strlen($forms_data['mineral_name']) > 20) {
				$returnValue = null;
			}
			if (strlen($forms_data['grade_code']) > 6) {
				$returnValue = null;
			}
			if (strlen($forms_data['grade_name']) > 100) {
				$returnValue = null;
			}
			if (!is_numeric($forms_data['grade_code'])) {
				$returnValue = null;
			}
			if (!preg_match('/^[a-zA-Z0-9\s]+$/', $forms_data['mineral_name'])) {
				$returnValue = null;
			}
			if (!preg_match("/^[0-9]+$/", $forms_data['grade_code'])) {
				$returnValue = null;
			}
			if (!preg_match('/^[A-Za-z0-9 \s%^]*$/', $forms_data['grade_name'])) {
				$returnValue = null;

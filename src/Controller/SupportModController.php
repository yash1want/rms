...
/**
 * This Controller all function written by - YS.
 * @addedon: 04th June 2023 (by Yashwant Singade)
 */
	...
	{	
		...
		$this->userSessionExits();
		 $this->viewBuilder()->setLayout('support_panel');
			...
			$d_count['rms_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['ticket_type'=>'RMS'])->count();
			$d_count['mpas_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['ticket_type'=>'MPAS'])->count();
$d_count['operational_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Operational'])->count();

			$d_count['db_related_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Database Related'])->count();
			
			$d_count['external_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'External'])->count();

			$d_count['training_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Training'])->count();

			$d_count['other_mod_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Other'])->count();

			$d_count['prog_related_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Program Related'])->count();

			$d_count['change_request_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Change Request'])->count();			
	...
	public function saveSupportLog($user, $status, $login_session=null, $userType) {
		...
		// $this->userSessionExits();		
		$loginemail = $this->Session->read('email');
		if($userType == 'supportuser')
		{
			...
			if($this->Session->read('support_team_login') == false){ // restrict support team logs
				$newEntity = $this->SupportLog->newEntity(array(
					'uname'=>base64_decode($loginemail),
					'ip_address'=>$ip_address,
					'login_time'=>$login_time,
					'status'=>$status,
					'session_token'=>$session_token
	...
	/**
	 * changes in
	 * @addedon: 05th July 2023 (by Yashwant Singade)
	 */
			            ...
			            if ($post_NewPwd == $post_ConfirmPwd) {
			              ...
			              //  $checkPassForComplexity = clsCommon::validatePassword($userPass);
			                ...
			                if ($updateQuery == '1') {
								...
								$this->saveSupportLog($userName,'SUCCESS','', $userTypee);
	...
	/*Yashwant added by 06-06-2023 Start- -------------*/
	...
	public function loginProceed($userName,$action,$userID)
	{
		...
		if($loginusertype=='supportuser')
		{
			...
			$support_data = $this->getSupportData($userID);
			...
			$designation = ($support_data['designation']) ? $support_data['designation'] : "";
			$role_id = ($support_data['role_id']) ? $support_data['role_id'] : "";
			...
			$this->Session->write('designation', $support_data['designation']);
			$this->Session->write('role_id', $support_data['role_id']);
	...
	/*Yashwant added by 06-06-2023 END- ---------------*/

	/*Yashwant added by 06-06-2023 Start- -------------*/
	public function getSupportData($userId){			

		$UserId = $userId;
		$support_data = $this->Support->find('all',array('conditions'=>array('id'=>$UserId)))->first();
	...
	public function ticketList() 
	{
		...
		if (null !== $this->request->getData('f_search')) 
		{
            ...
            {
    			/*Yashwant 27-06-2023 change start*/
				$from_date = $this->Customfunctions->getTicketDateByReturnFromRange($this->request->getData('from_date'));
    			/*Yashwant 27-06-2023 change end*/
	...
	public function ticketView()
	{
		...
		$issue_category = $getTicketDetails['issue_category'];												
		...
		$this->set('issue_category',$issue_category);
	...
	*/
	...
	public function viewInprocessTicket()
	{
		...
		$inproIssueTypes = array('Operational'=>'Operational','Database Related'=>'Database Related','Change Request'=>'Change Request','External'=>'External/Browser','Training'=>'Training','Program Related'=>'Program Related','Other'=>'Other');
	...
	public function viewResolvedTicket()
	{
		...
		//Supporting Documents---------13-06-2023 Start----------
		$getSupportingAttachments = $this->SupportingAttachments->find('all',array('conditions'=>array('token_number_id IS'=>$token_number)))->first();
         //echo"<pre>";print_r($getSupportingAttachments);exit;

		$support_add_more_attachment='';
		$support_add_more_description='';

		if (isset($getSupportingAttachments['add_more_attachment'])) 
	     {
       		$support_add_more_attachment = $getSupportingAttachments['add_more_attachment'];
    	 }
    	 if (isset($getSupportingAttachments['add_more_description'])) 
    	 {
        	$support_add_more_description = $getSupportingAttachments['add_more_description'];
    	 }
        
        $explodeSupportAttach = explode(',',$support_add_more_attachment);
        ...
        $explSupportDescript = explode(',',$support_add_more_description);
		...
		//Supporting Documents---------13-06-2023-----------END
	...
	public function allStatusFilter()
	{
		...
		/*$queryTickIssueCategory = $this->MmsRaisingTickets->find('list', [
			'keyField' => 'issue_category',
			'valueField' => 'issue_category',
		])
		->select(['issue_category'])->group(['issue_category'])->order(['issue_category' => 'ASC']);
		$tIssueCategory = $queryTickIssueCategory->toArray();
		$this->set('tIssueCategory', $tIssueCategory);*/


		$tIssueCategory =  array('Operational'=>'Operational','Database Related'=>'Database Related','Change Request'=>'Change Request','External'=>'External/Browser','Training'=>'Training','Program Related'=>'Program Related','Other'=>'Other');
	...
	public function reportStatusS1()
	{
		...
		if ($this->request->is('post')) 
		{
			...
			$issue_category = $this->request->getData('issue_category');
			...
			//$sql = "SELECT * FROM mms_raising_tickets mrt WHERE(mrt.created_at BETWEEN '$from_date' AND '$to_date'";
			$sql = "SELECT * FROM mms_raising_tickets AS mrt";
			...
			if ($issue_category != '') 
			{
				$sql .= " AND mrt.issue_category = '$issue_category'";
			}
			...
			foreach ($records as $record) 
			{
				...
				$allcount=$pending_count+$inprocess_count+$closed_count;

				$issue_category = $record['issue_category'];
				
				if($issue_category=='Operational')
				{
					$operational_count++;
				}
				elseif($issue_category=='Database Related')
				{
					$db_related_count++;
				}
				elseif($issue_category=='Change Request')
				{
					$change_request_count++;
				}
				elseif($issue_category=='External')
				{
					$external_count++;
				}
				elseif($issue_category=='Training')
				{
					$training_count++;
				}
				elseif($issue_category=='Program Related')
				{
					$prog_related_count++;
				}
				elseif($issue_category=='Other')
				{
					$other_mod_count++;
				}
			...
			$this->set('pending_count', $pending_count);
			$this->set('inprocess_count', $inprocess_count);
			$this->set('closed_count', $closed_count);

			$this->set('operational_count', $operational_count);
			$this->set('db_related_count', $db_related_count);
			$this->set('change_request_count', $change_request_count);
			$this->set('external_count', $external_count);
			$this->set('training_count', $training_count);
			$this->set('prog_related_count', $prog_related_count);
			$this->set('other_mod_count', $other_mod_count);

			$this->set('rms_count', $rms_count);
			$this->set('mpas_count', $mpas_count);


			/*$this->loadModel('MmsRaisingTickets');
	 		$allcount = $this->MmsRaisingTickets->find()->select(['id'])->count();
	 		$pending_count = $this->MmsRaisingTickets->find()->select(['id'])->where(['status'=>'Pending'])->count();
	 		$inprocess_count = $this->MmsRaisingTickets->find()->select(['id'])->where(['status'=>'Inprocess'])->count();
	 		$closed_count = $this->MmsRaisingTickets->find()->select(['id'])->where(['status'=>'Closed'])->count();

			$d_count['operational_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Operational'])->count();
		 	$d_count['db_related_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'DB_Related'])->count();
		 	$d_count['api_related_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'API_Related'])->count();
		 	$d_count['external_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'External'])->count();
		 	$d_count['training_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Training'])->count();
		 	$d_count['other_mod_count'] = $this->MmsRaisingTickets->find()->select(['id'])->where(['issue_category'=>'Other_Module_Related'])->count();
	 		...
	 		if (!empty($records)) {
	 			$lprint = $this->generateTicketS1($records, $showDate, $rowCount,$allcount,$pending_count,$inprocess_count,$closed_count);
				//echo"<pre>";print_r($lprint);exit;
			...
			} else 
			{
				...
				$alert_redirect_url = "all-status-filter?title=report-filter";
				$alert_theme = "success";
	...
	//-------------------------------------------------------------------------------
	public function generateTicketS1($records, $showDate, $rowCount,$allcount,$pending_count,$inprocess_count,$closed_count)
	{
		//print_r(date('Y-m-d H:i:sa'));exit;
		//$currentDate = date("Y-m-d");
		//$currentDate = date('Y-m-d H:i:s', strtotime('-1 day'));
		//echo "Current date: " . $currentDate;exit;
		$this->userSessionExits();
		$this->viewBuilder()->setLayout('ticket_report_layout');
		$datarry = array();
		$lcnt = 1;
		...
		<div class="card-body">
		<h6 class="text-center"  id="heading2"><strong><u>Date : From ' . $showDate . '</u></strong></h6>
		<!--tHeadDate <input type="button" id="downloadExcel" value="Export to Excel">-->

		<br/>
		<div class="table-responsive">
		<table id="noDatatable" class="table table-striped table-bordered" style="width:100%">
		...
		<!-- Graph ticket Status  Start id="noDatatable"-->
		<div class="row">
		<div class="col-md-4">
		...
		<!-- Ticket status -->
		<h4 class="widget-heading font-weight-bold font_12">Tickets Status</h4>
		...
		<canvas id="donutChart" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%; display: block; width: 764px;" width="764" height="250" class="chartjs-render-monitor"> width="764" height="250" class="chartjs-render-monitor"></canvas>
		</div>
		</div>
		</div>
		</div>
		...
		<h4 class="widget-heading font-weight-bold font_12 text-white">Ticket Issue Type</h4>
		<div class="card-tools">
		<button type="button" class="btn btn-tool" data-card-widget="collapse">
		<i class="fas fa-minus"></i>
		</button>
		...
		<canvas id="pieChartTickIssueType" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%; display: block; width: 764px;" width="764" height="250" class="chartjs-render-monitor"> width="764" height="250" class="chartjs-render-monitor">
		</canvas>
		</div>
		</div>
		</div>
		...
		<div class="card-header">
		<!-- <h3 class="card-title">Pie Chart</h3> -->
		<h4 class="widget-heading font-weight-bold font_12 text-white">Ticket Module</h4>
		<div class="card-tools">
		<button type="button" class="btn btn-tool" data-card-widget="collapse">
		<i class="fas fa-minus"></i>
		</button>
		...
		<div class="card-body"><div class="chartjs-size-monitor"><div class="chartjs-size-monitor-expand"><div class=""></div></div><div class="chartjs-size-monitor-shrink"><div class=""></div></div></div>
		<canvas id="pieChartTicketModule" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%; display: block; width: 764px;" width="764" height="250" class="chartjs-render-monitor"> width="764" height="250" class="chartjs-render-monitor">
		</canvas>
		</div>
		</div>
		</div>
		...
		<!-- All Status Count Start-->
		<div class="row">
		<div class="col-md-3">
		<h6><button type="button" class="text-center btn btn-primary font-weight-bold font_12 "><!--mrleft-->
		All Ticket Status <span class="badge badge-light font-weight-bold font_12 ">'.$allcount.'</span>
		</button>
		</h6>
		</div>
		<div class="col-md-3">
		<h6><button type="button" class="text-center btn btn-warning font-weight-bold font_12 "><!--mrleft-->
		Pending Ticket Status <span class="badge badge-light font-weight-bold font_12 ">'.$pending_count.'</span>
		</button>
		</h6>
		</div>
		<div class="col-md-3">
		<h6><button type="button" class="text-center btn btn-info font-weight-bold font_12 flright">
		Inprocess Ticket Status <span class="badge badge-light font-weight-bold font_12 ">'.$inprocess_count.'</span>
		</button>
		</h6>
		</div>
		<div class="col-md-3">
		<h6><button type="button" class="text-center btn btn-success font-weight-bold font_12  flright">
		Closed Ticket Status <span class="badge badge-light font-weight-bold font_12 ">'.$closed_count.'</span>
		</button>
		</h6>
		</div>
		<div>
		<!-- All Status Count End-->
		...
		<th rowspan="2">Ticket Issue Category</th>
		...
		foreach ($records as $record) 
		{
			...
			if ($lissue_category != $record['issue_category']) {
				$lissue_category = $record['issue_category'];
				$lflg = "Y";
			}
			...
			{ 
				...
				//-- Calculate Day Count --
				$oldDate = date("d-m-Y", strtotime($created_at));
				$replyDate = date("d-m-Y", strtotime($record['reply_date']));
				...
				if ($lcnt >= 0) 
				{
					...
					$totalstatusCount=$pendingCount+$inprocessCount+$closedCount;
					...
					<td ' . $lrowspan . '>' . $issue_category . '</td>
					<td ' . $lrowspan . '>' . $issued_raise_at . '</td>												
					<td ' . $lrowspan . '>' . $issued_type . '</td>
					<td ' . $lrowspan . '>' . $status . '</td>
					<td ' . $lrowspan . '>
					<h6><span class="badge badge-pill badge-dark font-weight-bold">' . $count . '</span></h6>
					</td>
		...
		$this->set('inprocessCount', $inprocessCount);
		$this->set('closedCount', $closedCount);
		$this->set('totalstatusCount', $totalstatusCount);
	...
	/*================= Start Showing Support-Logs Details 05-07-2023 Yashwant =======================*/
	public function logs()
	{
		$userId = base64_decode($this->Session->read('email'));
		$this->autoRender = false;
		$this->userSessionExits();
		$this->viewBuilder()->setLayout('support_panel');
		...
		$this->loadModel('SupportLog');
		$logdata = $this->SupportLog->find('all')->where(['uname'=>$userId])->where(['DATE(login_time) BETWEEN :start AND :end'])->bind(':start',$from_date,'date')->bind(':end',$to_date,'date')->order(['id DESC'])->toArray();
		$this->set('from_date',$from_date);
		$this->set('to_date',$to_date);
		$this->set('log_filtered_txt',$log_filtered_txt);
		$this->set('logdata',$logdata);
		$this->render('/Support/logs');
	}
	/*================= END Showing Support-Logs Details 04-07-2023 Yashwant =======================*/

...
class ReturnspdftestController extends AppController{

    var $name = 'Returnspdftest';
	var $uses = array();
    ...
    public function minerPrintPdf(){
		...
		foreach($mineralArr as $key=>$val){
			...
			$saleDespatchGrade[$mineral] = $this->Clscommon->getGradeArr(strtoupper(str_replace('_', ' ', $mineral)));
		...
		$this->generateReturnsPdf('/Returnspdftest/minerPrintPdf');
    ...
    public function enduserPrintPdf(){
		...
		//call custome function from appcontroller to create pdf
		$this->generateReturnsPdf('/Returnspdf/enduserPrintPdf');
	...
	public function generateReturnsPdf($pdf_view_path){

		$applicantid = str_replace('/','-',$this->Session->read('applicantid'));
		$loginusertype = $this->Session->read('loginusertype');
		...
		//print_r($all_data_pdf); exit;
		

		//check applicant last record version to increment		
		$list_id = $this->EsignPdfRecords->find('all', array('fields'=>'version', 'conditions'=>array('applicant_id IS'=>$applicantid,'return_type'=>$returnDate,'return_date'=>$returnType)))
												->order('id desc')
												->limit(1)
												->first();
		...
		$this->Session->write('pdf_file_name',$pdfPrifix.'-'.$applicantid.'('.$current_pdf_version.')'.'.pdf');
	...
	public function pdfFilePath(){
		...
		$filename .= '/writereaddata/returns/';

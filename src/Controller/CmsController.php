...
class CmsController extends AppController{
	...
	//authenticate user to access cms 
	public function authenticateUserForCms() {

		$user_access = $this->Session->read('user_roles');
		if (in_array('cms',$user_access)) {
			//proceed
		} else {
		...
		if (null !== ($this->request->getData('upload'))) {
			...
			if (!empty($check_duplicate_filename)) {
				...
				if (!empty($this->request->getData('file')->getClientFilename())) {
					...
					$file = $this->Customfunctions->fileUploadLib($file_name,$file_size,$file_type,$file_local_path,15);
					//print_r($file);die;
